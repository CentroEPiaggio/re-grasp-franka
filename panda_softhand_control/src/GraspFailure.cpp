...
GraspFailure::GraspFailure(ros::NodeHandle& nh_){
    ...
    ros::Subscriber network_sub = this->nh.subscribe("/output_NN", 1, &GraspFailure::get_network_state, this);
...
bool GraspFailure::parse_task_params(){
    ...
    if(!ros::param::get("/grasp_params_1/home_grasp_transform_1", this->home_grasp_transform_1)){
	    ROS_WARN("The param 'home_grasp_transform_1' not found in param server! Using default.");
		this->home_grasp_transform_1.resize(6);
        std::fill(this->home_grasp_transform_1.begin(), this->home_grasp_transform_1.end(), 0.0);
		success = false;
	}

    // Converting the home_grasp_transform_1 vector to geometry_msgs Pose
    this->home_grasp_T = this->convert_vector_to_pose(this->home_grasp_transform_1);
...
void GraspFailure::get_network_state(const std_msgs::Float32::ConstPtr &msg){

    // Saving the message
    this->latest_network_state = *msg;
...
bool GraspFailure::call_simple_grasp_task(std_srvs::SetBool::Request &req, std_srvs::SetBool::Response &res){
    ...
    Eigen::Affine3d home_grasp_transform_1_aff; tf::poseMsgToEigen(this->home_grasp_T, home_grasp_transform_1_aff);

    geometry_msgs::Pose pre_grasp_pose; geometry_msgs::Pose home_grasp_pose; geometry_msgs::Pose grasp_pose;
    tf::poseEigenToMsg(object_pose_aff * pre_grasp_transform_1_aff, pre_grasp_pose);
    tf::poseEigenToMsg(object_pose_aff * home_grasp_transform_1_aff, home_grasp_pose);
    ...
    if(!this->panda_softhand_client_1.call_pose_service(pre_grasp_pose, present_pose, false, this->tmp_traj, this->tmp_traj) || !this->franka_ok || !this->network_ok){
    ...
    if(!this->panda_softhand_client_1.call_arm_control_service(this->tmp_traj) || !this->franka_ok){
     ...
     std::cout << "Output NN is \n" << this->network_ok << std::endl;
     
     // 2) Going to grasp pose
     if(!this->panda_softhand_client_1.call_pose_service(grasp_pose, pre_grasp_pose, false, this->tmp_traj, this->tmp_traj) || !this->franka_ok || !this->network_ok){
     ...
     if(!this->panda_softhand_client_1.call_slerp_service(pre_grasp_pose, grasp_pose, false, this->tmp_traj, this->tmp_traj) || !this->franka_ok || !this->network_ok){
         ROS_ERROR("Could not plan to the specified grasp pose.");
         res.success = false;
         res.message = "The service call_simple_grasp_task was NOT performed correctly!";
         return false;
     ...
     // 5) Returning to home grasp pose
     if(!this->panda_softhand_client_1.call_slerp_service(home_grasp_pose, pre_grasp_pose, false, this->tmp_traj, this->tmp_traj) || !this->franka_ok || !this->network_ok){
         ROS_ERROR("Could not plan to the specified grasp pose.");
         res.success = false;
         res.message = "The service call_simple_grasp_task was NOT performed correctly!";
         return false;
