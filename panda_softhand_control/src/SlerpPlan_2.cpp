...
SlerpPlan_2::SlerpPlan_2(ros::NodeHandle& nh_, std::string group_name_, std::string end_effector_name_,  int n_wp_){
        ...
        this->nh = nh_;
        group = new moveit::planning_interface::MoveGroupInterface(group_name_);
        // this->mygroup=group;
        // Initializing names
...
bool SlerpPlan_2::initialize(geometry_msgs::Pose goal_pose, geometry_msgs::Pose start_pose, bool is_goal_relative, trajectory_msgs::JointTrajectory past_trajectory){
    ...
    try {
		this->tf_listener.waitForTransform("/world", this->end_effector_name, ros::Time(0), ros::Duration(50.0) );
...
bool SlerpPlan_2::performMotionPlan(){
    ...
    ros::Time time_pre;
    ros::Time time_post;   
    time_pre=ros::Time::now();
    std::cout << "time_pre PERFORM MOTION PLANNING \n" << time_pre << std::endl;  
    
    // // Move group interface
    // moveit::planning_interface::MoveGroupInterface group(this->group_name);
    
    time_post=ros::Time::now();
    std::cout << "time_post PERFORM MOTION PLANNING \n" << time_post << std::endl;   
	...
	double fraction = group->computeCartesianPath(cart_waypoints, 0.2, 0.0, trajectory);
