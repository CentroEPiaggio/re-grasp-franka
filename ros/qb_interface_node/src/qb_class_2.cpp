...
/*
/ *****************************************************
/ Costructor of qb_class_2 class
/ *****************************************************
/   parameters:
...
qb_class_2::qb_class_2(){
	...
	if (!hand_chain_.empty()){
		...
		// Subscriber initialize	
 		hand_sub = node_->subscribe("/qb_class_2/hand_ref", 1, &qb_class_2::handRefCallback, this); 	
 		hand_CurrAndPos_sub = node_->subscribe("/qb_class_2/hand_ref_curr_and_pos", 1, &qb_class_2::handRefCurrAndPosCallback, this);	

		// Publisher initialize

		// Outside command publisher, self-open but not internally used topic
		handRef_pub = node_->advertise<qb_interface::handRef>("/qb_class_2/hand_ref", 1);
		handRef_CurrAndPos_pub = node_->advertise<qb_interface::handRef_CurrAndPos>("/qb_class_2/hand_ref_curr_and_pos", 1);

		// Publisher to publish new read positions
		hand_pub = node_->advertise<qb_interface::handPos>("/qb_class_2/hand_measurement", 1);
		hand_pub_grasp = node_->advertise<std_msgs::Float64>("/qb_class_2/hand_measurement/grasp_failure", 1);

		// Current TOPIC

		if (flag_curr_type_)
			hand_curr_pub = node_->advertise<qb_interface::handCurrent>("/qb_class_2/hand_current", 1);

	  	//service callbacks
  		srv_control_pid_ = node_->advertiseService("control_pid", &qb_class_2::change_control_pid, this);
	}

	if (!cube_chain_.empty()){

		// Subscriber initialize
		cube_sub = node_->subscribe("/qb_class_2/cube_ref", 1000, &qb_class_2::cubeRefCallback, this);
		// Publisher initialize

		// Outside command publisher, self-open but not internally used topic
		cubeRef_pub = node_->advertise<qb_interface::cubeRef>("/qb_class_2/cube_ref", 1);

		// Publisher to publish new read positions
		if (flagCMD_type_ == EQ_PRESET)
			cube_pub = node_->advertise<qb_interface::cubeEq_Preset>("/qb_class_2/cube_measurement", 1);
		else
			cube_pub = node_->advertise<qb_interface::cubePos>("/qb_class_2/cube_measurement", 1);

		// Current TOPIC
		if (flag_curr_type_)
			cube_curr_pub = node_->advertise<qb_interface::cubeCurrent>("/qb_class_2/cube_current", 1);
	}
...
/*
/ *****************************************************
/ Descructor of qb_class_2 class
/ *****************************************************
/   parameters:
/   return:
/
...
bool qb_class_2::open(const char* port, const int br) {
...
void qb_class_2::handRefCallback(const qb_interface::handRef::ConstPtr& msg){
	pos_ = msg->closure;
}
...
void qb_class_2::handRefCurrAndPosCallback(const qb_interface::handRef_CurrAndPos::ConstPtr& msg){
	pos_ = msg->closure;
...
void qb_class_2::cubeRefCallback(const qb_interface::cubeRef::ConstPtr& msg){
	p_1_ = msg->p_1;
	p_2_ = msg->p_2;
}
...
void qb_class_2::sendHandMeas(vector<float> closure){
    ...
    std_msgs::Float64 closure_data;
    closure_data.data = closure[0]; 
 
    //hand_value.data
    hand_pub_grasp.publish(closure_data);
...
void qb_class_2::sendCubeMeas(vector<float> eq, vector<float> preset){
...
void qb_class_2::sendCubeMeas(vector<float> pos_1, vector<float> pos_2, vector<float> pos_L){
...
void qb_class_2::sendCurrent(vector<int> current_m1, vector<int> current_m2){
...
void qb_class_2::sendCurrent(vector<int> current){
...
bool qb_class_2::readMeasCurrent() {
...
// Service
bool qb_class_2::change_control_pid(qb_interface::control_pid::Request& req, qb_interface::control_pid::Response& res){
